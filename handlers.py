import os
"""
handlers.py — Интерактивное меню настроек бота через inline-кнопки в Telegram.
...
def _offer_url(offer_id):
    base = os.environ.get('TARGET_BASE_URL', 'https://example.com')
...
async def _show_minprice_section_menu(query, context, section='all', back_cb='set:check:menu'):
    ...
    if section == 'skins':
        ...
        text = "💰 <b>Мін. прайс тест</b>\n\nВыберите позицию для поиска минимальной цены на Market:"
...
async def handle_settings_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === Filters ===
        ...
        if action == 'list':
            ...
            await query.edit_message_text(
                '🚷 <b>Добавить в бан</b>\n\nОтправьте ссылку на лот или ID лота.\n\nПример:\n<code>https://market.com/lots/offer?id=65475872</code>\n\nИли /cancel',
                parse_mode='HTML'
            )
        elif action == 'rm':
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == INPUT_SKIN_PRICE:
        ...
        if not offer_id:
            await update.message.reply_text('❌ Отправьте ссылку на лот Market или ID. Или /cancel:')
            return
        banned_ids = _get_banned_ids(context)
