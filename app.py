...
# URLs с фильтром "только продажа" (несколько категорий Game)
...
def _sync_get_offer_details(offer_url):
    ...
    try:
        ...
        time.sleep(delay)
        with build_http_session() as session:
            session.headers.update({'Referer': 'https://market.com/'})
...
def _sync_search_min_price(keywords, require_pve=False):
    """Ищет минимальную цену по ВСЕМ ключевым словам через поиск Market."""
    ...
    with build_http_session() as session:
        for kw in keywords:
            search_url = f'{TARGET_URL}&search={requests.utils.quote(kw)}'
            try:
...
async def _process_offers_impl(bot_instance=None, context=None, skip_seen=True, max_price_override=None,
    ...
    try:
        ...
        set_check_progress(
            ...
            total=1,
            current="Получаю список лотов с Market",
            sent=0,
        )
...
async def ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not ids_to_add:
        await update.message.reply_text("❌ Не нашёл валидных ID или ссылок. Пример: /ban https://example.com/lots/offer?id=62283710")
        return
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    help_text = (
        f"📖 <b>Справка Market Monitor</b>\n\n"
        f"Бот мониторит аккаунты Game на Market и присылает уведомления "
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🔍 Ищу аккаунты Game с подтверждённым STW/PVE и редкими скинами.\n"
...
def _log_startup_banner(mode):
    ...
    logger.info("╔══════════════════════════════════════════════╗")
    logger.info("║           Market Monitor Bot                ║")
    logger.info("╠══════════════════════════════════════════════╣")
...
def main():
    ...
    job_queue.run_repeating(check_market_job, interval=config.check_interval, first=10)
