...
async def _process_offers_impl(bot_instance=None, context=None, skip_seen=True, max_price_override=None,
    ...
    try:
        ...
        for idx, candidate in enumerate(candidates[:limit], start=1):
            ...
            # Calculate helper variables for the table
            is_seller_ok = True
            if not rating_text or "0 отзывов" in rating_text or "Ошибка" in rating_text or "❗" in rating_text or "Нет рейтинга" in rating_text or "❌" in rating_text:
                is_seller_ok = False
